Shader "Learn/Surface/Wobble"
{
    ...
    SubShader {
        ...
        void vert(inout appdata_full data){
            ...
            modifiedPos.x += sin(data.vertex.x * _Frequency + _Time.y * _AnimationSpeed) * _Amplitude;
            
            float3 posPlusTangent = data.vertex + data.tangent * 0.01;
            posPlusTangent.x += sin(posPlusTangent.x * _Frequency + _Time.y * _AnimationSpeed) * _Amplitude;

            float3 bitangent = cross(data.normal, data.tangent);
            float3 posPlusBitangent = data.vertex + bitangent * 0.01;
            posPlusBitangent.y += sin(posPlusBitangent.y * _Frequency + _Time.y * _AnimationSpeed) * _Amplitude;
